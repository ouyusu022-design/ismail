...
# --- 5. واجهة التطبيق الرئيسية ---
    ...
    if choice == "📋 عرض والبحث حسب نوع الرخصة":
        ...
        if not filtered_df.empty:
            # تصدير البيانات إلى ملف Excel حقيقي (.xlsx) بجداول وأعمدة منسقة
...
"""
    ...
    # --- 7. تعديل أو حذف ---
        ...
        if df.empty:
            ...
            new_notes = st.text_area("ملاحظات:", value=str(row_data["ملاحظات"]))
            ...
            with col_btn1:
                if st.button("🔄 تحديث البيانات"):
                    ...
                    save_data(df)
                    st.success("✅ تم تحديث بيانات الرخصة!")
                    st.rerun()
            ...
            with col_btn2:
                if st.button("🗑️ حذف هذه الرخصة"):
                    ...
                    save_data(df)
                    st.warning("🗑️ تم حذف الرخصة!")
                    st.rerun()

    # --- 8. تغيير كلمة السر ---
    elif choice == "🔑 تغيير كلمة السر الخاصة بي":
        st.subheader("🔑 تغيير كلمة السر")
        
        with st.form("change_pass_form"):
            curr_pass = st.text_input("كلمة السر الحالية:", type="password")
            new_pass = st.text_input("كلمة السر الجديدة:", type="password")
            confirm_pass = st.text_input("تأكيد كلمة السر الجديدة:", type="password")
            
            change_btn = st.form_submit_button("🔄 تحديث كلمة السر")
            
            if change_btn:
                username = user["username"]
                if users_db[username]["password"] != curr_pass:
                    st.error("❌ كلمة السر الحالية غير صحيحة.")
                elif new_pass != confirm_pass:
                    st.error("⚠️ كلمة السر الجديدة وتأكيدها غير متطابقين.")
                elif len(new_pass) < 4:
                    st.error("⚠️ كلمة السر ضعيفة جداً، المرجو كتابة 4 أرقام/حروف على الأقل.")
                else:
